...
/**
 * Program completion notification for related users.
 *
 ...
 */
final class completion_relateduser extends base {
    /**
     * Send notifications.
     *
     * @param stdClass|null $program
     * @param stdClass|null $user allocated user
     * @return void
     */
    public static function notify_users(?stdClass $program, ?stdClass $user): void {
        ...
        $sql = "SELECT pa.*, rur.parent as relateduserid
                  ...
                  JOIN {local_openlms_notifications} n
                       ON n.component = 'enrol_programs' AND n.notificationtype = 'completion_relateduser' AND n.instanceid = p.id AND n.enabled = 1
        ...
        foreach ($rs as $allocation) {
            ...
            self::notify_related_user($program, $source, $allocation, $user, $relateduser, true);
        }
        $rs->close();
    }

    /**
     * Send notifications related to allocation to related user.
     *
     * @param stdClass $user
    ...
    public static function notify_now(stdClass $user, stdClass $program, stdClass $source, stdClass $allocation): void {
        $relateduser = self::get_relateduser($allocation->userid);
        if (!$relateduser) {
            return;
        }
